...
export default function SmartWithdrawalModal({ isOpen, onClose, eqBalance, eqPrice }: SmartWithdrawalModalProps) {
  ...
  const handleTransferToFutures = async () => {
    ...
    try {
      ...
      await supabase.from('user_mining_data').update({
        total_mined: eqBalance - parseFloat(withdrawAmount),
        updated_at: new Date().toISOString()
      }).eq('user_id', user.id);

      await supabase.from('user_balances').update({
        futures_balance: supabase.raw(`futures_balance + ${futuresTotal + welcomeBonus}`),
        updated_at: new Date().toISOString()
      }).eq('user_id', user.id);
  ...
  const handleWithdrawToWallet = async () => {
    ...
    try {
      ...
      await supabase.from('user_balances').update({
        usdt_balance: supabase.raw(`usdt_balance + ${walletTotal}`),
        updated_at: new Date().toISOString()
      }).eq('user_id', user.id);
