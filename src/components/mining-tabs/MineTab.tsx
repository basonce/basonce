...
export default function MineTab({ onSwitchToShop }: { onSwitchToShop?: () => void }) {
  ...
  const initDemoMode = () => {
    ...
    // Check if demo already started
    const demoStartTime = localStorage.getItem('demo_mining_start');
    const demoMaxDuration = 15 * 60; // 15 minutes in seconds

    if (demoStartTime) {
      const elapsed = Math.floor((Date.now() - parseInt(demoStartTime)) / 1000);
      const remaining = Math.max(0, demoMaxDuration - elapsed);

      if (remaining > 0) {
        setDemoTimeRemaining(remaining);

        // Load demo miner from localStorage
        const savedDemo = localStorage.getItem('demo_miner');
        if (savedDemo) {
        ...
        // Demo expired, clear and restart
        localStorage.removeItem('demo_mining_start');
        localStorage.removeItem('demo_miner');
      }
    }

    // Create demo CPU miner - FAST 5 HOUR DOPAMINE HIT!
    const demoMiner: MinerDevice = {
      id: 'demo-cpu-miner',
      name: 'CPU Miner',
      icon: '💻',
      hash_rate: 10,
      daily_earning_usdt: 624, // $624/day * 5h/24 = $130 total
      total_earned_usdt: 0,
      ...
      level: 0,
      mining_duration_hours: 0.25, // 15 minutes (demo mode)
      ...
      return prev.map(m => {
        if (m.status === 'active') {
          ...
          if (m.has_time_limit && newDuration >= maxDuration) {
            ...
            // Show upgrade decision screen
            setTimeout(() => {
              const shouldUpgrade = confirm(
                `🎉 Free Mining Complete!\n\n` +
                `Your CPU Miner has finished its 5-hour session.\n\n` +
                `💰 Total Earned: $${m.session_earned_usdt.toFixed(2)} USDT\n` +
                `⏱️ Time Used: ${(maxDuration / 3600).toFixed(1)} hours\n\n` +
                `⚠️ This equipment will be deactivated after you collect.\n\n` +
                `Next Steps:\n` +
                `1️⃣ Collect your earnings ($${m.session_earned_usdt.toFixed(2)} USDT)\n` +
                `2️⃣ Visit Shop to buy unlimited mining equipment\n\n` +
                `Would you like to view the Shop now?`
              );
  ...
  // ✅ COLLECT: Transfer session earnings to database balance (ATOMIC)
  // ⚠️ CRITICAL: ONLY ADD USDT, NOT EQ! EQ is just for display, not real balance
  const handleCollectEarnings = async () => {
    ...
    // ✅ CHECK IF ANY EQUIPMENT HAS REACHED ITS WITHDRAWAL LIMIT
    const minersWithEarnings = miners.filter(m => m.session_earned_usdt > 0);

    // Check each miner's individual limit
    for (const miner of minersWithEarnings) {
      const totalFromThisMiner = miner.total_earned_usdt + miner.session_earned_usdt;
      if (totalFromThisMiner >= miner.withdrawal_limit) {
        alert(
          `⚠️ Equipment Limit Reached!\n\n` +
          `${miner.icon} ${miner.name} has reached its withdrawal limit of $${miner.withdrawal_limit.toLocaleString()}!\n\n` +
          `Current Earnings: $${totalFromThisMiner.toFixed(2)} USDT\n\n` +
          `This equipment can no longer be used.\n` +
          `Collect your earnings and upgrade to higher tier equipment in the Shop!`
        );
        return;
      }
    }
    ...
    const confirmed = confirm(
      ...
      `(EQ: ${sessionEqTotal.toFixed(4)} EQ is for display only)\n\n` +
    ...
    try {
      ...
      // ✅ ONLY ADD USDT - EQ is just display metric, not real balance
      const newUsdtBalance = dbUsdtBalance + sessionUsdtTotal;

      await supabase
        .from('user_balances')
        .update({
          balance: Number(newUsdtBalance.toFixed(4))
          // ❌ NO eq_amount update! EQ is just for show
        })
        .eq('user_id', user.id)
        .eq('symbol', 'USDT');

      // Update each miner's total_earned_from_equipment
      const equipmentWithEarnings = miners.filter(m => m.session_earned_usdt > 0);

      for (const miner of equipmentWithEarnings) {
        const newTotalFromEquipment = miner.total_earned_usdt + miner.session_earned_usdt;

        // ✅ CPU Miner (time-limited free equipment): DEACTIVATE after collect
        // Other equipment: can be used again (reset timer)
        const isCPUMiner = miner.name === 'CPU Miner' && miner.has_time_limit;
        const maxSeconds = miner.mining_duration_hours * 3600;
        const hasReachedTimeLimit = miner.used_mining_seconds >= maxSeconds;

        await supabase
          .from('user_mining_equipment')
          .update({
            session_earned_usdt: 0,
            total_earned_from_equipment: newTotalFromEquipment,
            is_active: isCPUMiner && hasReachedTimeLimit ? false : true,
            started_at: null,
            used_mining_seconds: isCPUMiner ? miner.used_mining_seconds : 0,
            ends_at: null,
            status: 'stopped'
          })
          .eq('id', miner.id)
      ...
      // Update local state - ONLY USDT
      setDbUsdtBalance(newUsdtBalance);
      // dbEqBalance stays same (not increased)
      setMiners(prev => prev.map(m => {
        if (m.session_earned_usdt > 0) {
          const maxDuration = m.mining_duration_hours * 3600;
          const isCPU = m.name === 'CPU Miner' && m.has_time_limit;
          const hasReachedLimit = m.used_mining_seconds >= maxDuration;

          // CPU Miner with time limit reached: Remove from active list
          if (isCPU && hasReachedLimit) {
            return {
              ...m,
              session_earned_usdt: 0,
              status: 'stopped' as const,
          ...
          // CPU Miner still has time: Keep used_mining_seconds
          if (isCPU) {
            return {
              ...m,
              session_earned_usdt: 0,
              status: 'stopped' as const,
              remaining_mining_seconds: maxDuration - m.used_mining_seconds,
              usage_percentage: (m.used_mining_seconds / maxDuration) * 100,
              started_at: null
            };
          }

          // Other equipment: Reset for reuse
          return {
            ...m,
            session_earned_usdt: 0,
            status: 'stopped' as const,
            used_mining_seconds: 0,
            remaining_mining_seconds: m.has_time_limit ? maxDuration : 999999,
            usage_percentage: 0,
            started_at: null
          };
        }
      ...
      // Check if CPU Miner was deactivated
      const cpuMinerDeactivated = equipmentWithEarnings.some(m => {
        const isCPU = m.name === 'CPU Miner' && m.has_time_limit;
        const maxSeconds = m.mining_duration_hours * 3600;
        return isCPU && m.used_mining_seconds >= maxSeconds;
      });

      if (cpuMinerDeactivated) {
        const shouldUpgrade = confirm(
          `✅ Mining Earnings Collected!\n\n` +
          `+$${sessionUsdtTotal.toFixed(2)} USDT\n` +
          `New Balance: $${newUsdtBalance.toFixed(2)} USDT\n\n` +
          `🎉 Congratulations on completing your free mining session!\n\n` +
          `⚠️ CPU Miner has been deactivated (5-hour limit reached)\n\n` +
          `Would you like to view premium mining equipment?\n` +
          `Shop has powerful miners with unlimited usage!`
        );
  ...
  const toggleMiner = async (minerId: string, currentStatus: string) => {
    ...
    supabase
      ...
      .eq('id', minerId)
      .then(() => {
        console.log('Mining status updated in database');
      })
      .catch(error => {
