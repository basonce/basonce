...
export default function GamesTab() {
  ...
  const loadBigWins = async () => {
    const { data, error } = await supabase.rpc('get_recent_big_wins', { win_limit: 10 });
    if (data) setBigWins(data);
  };
  ...
  const playCoinFlip = async () => {
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          user_id: user.id,
          game_type_id: selectedGame!.id,
          bet_amount: bet,
          multiplier: multiplier,
          payout: payout,
          profit: profit,
          game_data: { choice: coinChoice, result: result }
        });
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          payout: payout,
          profit: profit,
          game_data: { target: diceTarget, roll: roll }
        });
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          payout: payout,
          profit: profit,
          game_data: { cashout: cashoutMultiplier, crash_point: crashPoint.toFixed(2) }
        });
      ...
      if (hitMine) {
        ...
        await supabase
          ...
          .insert({
            ...
            payout: 0,
            profit: -bet,
            game_data: { mines_count: minesCount, revealed: newRevealed.length }
          });
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          payout: payout,
          profit: profit,
          game_data: { mines_count: minesCount, revealed: safeRevealed, cashed_out: true }
        });
