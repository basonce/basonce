...
function generateSimulatedKlines(basePrice: number, count: number, intervalMs: number): KlineData[] {
  ...
  let price = basePrice * (0.7 + Math.random() * 0.2);
  ...
  for (let i = count; i > 0; i--) {
    ...
    data.push({ time, open, high, low, close: Math.max(close, low * 1.001), volume });
...
function BinanceLightweightChart({ symbol, binanceSymbol, timeframe, currentPrice }: Props) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (symbol === 'EQ') {
          const eqPrice = EarnQuestPriceManager.getInstance().getPrice() || 0.055;
          klineData = generateSimulatedKlines(eqPrice, limit, intMs);
        } else {
          ...
          if (klineData.length === 0) {
            ...
            try {
              ...
              if (ticker && ticker.lastPrice) {
                fallbackPrice = parseFloat(ticker.lastPrice);
              }
            } catch {}
            ...
            if (fallbackPrice <= 0) {
              for (let i = 0; i < 10; i++) {
                ...
                if (currentPriceRef.current > 0) {
                  fallbackPrice = currentPriceRef.current;
                  break;
                }
              }
            }
  ...
  useEffect(() => {
    ...
    if (lastKline && Math.abs(currentPrice / lastKline.close) > 10) return;
    if (symbol === 'EQ') {
      const eqPrice = EarnQuestPriceManager.getInstance().getPrice() || 0.055;
      if (currentPrice > eqPrice * 50) return;
    }
    const now = Math.floor(Date.now() / 1000);
