        ...
        {positions.slice(0, 4).map((pos, i) => (
          <div key={i} className="bg-[#1E2026] rounded-lg p-3 border border-[#2B3139]">
            ...
            <div className="grid grid-cols-2 gap-1 text-[8px]">
              <div>
                <div className="text-gray-500">Size ({pos.coin})</div>
                <div className="text-white font-medium">{(pos.size / (pos.entry || 1)).toLocaleString('en-US', { maximumFractionDigits: 1 })}</div>
              </div>
              <div>
                <div className="text-gray-500">Margin (USDT)</div>
                <div className="text-white font-medium">{pos.margin.toLocaleString('en-US', { maximumFractionDigits: 2 })}</div>
              </div>
              <div>
                <div className="text-gray-500">Entry Price (USDT)</div>
                <div className="text-white font-medium">{formatPrice(pos.entry)}</div>
              </div>
              <div>
                <div className="text-gray-500">Mark Price (USDT)</div>
                <div className="text-white font-medium">{formatPrice(pos.mark)}</div>
              </div>
              <div>
                <div className="text-gray-500">Liq. Price (USDT)</div>
