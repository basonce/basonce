    ...
    return (
      <div className="bg-[#1E2026] rounded-lg p-3 border border-[#2B3139]">
        ...
        <div className="mb-2">
          ...
          <div className={`text-lg font-bold ${isBullish ? 'text-[#0ECB81]' : 'text-[#F6465D]'}`}>
            {isBullish ? '+' : ''}{profitLoss.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
          <div className="flex items-center gap-1 mt-0.5">
            <span className="text-[9px] text-gray-400">ROI</span>
            <span className={`text-[11px] font-semibold ${isBullish ? 'text-[#0ECB81]' : 'text-[#F6465D]'}`}>
  ...
  return (
    <div className="bg-[#1E2026] rounded-xl p-4 mb-3 border border-[#2B3139]">
      ...
      <div className="mb-4">
        ...
        <div className={`text-3xl font-bold ${isBullish ? 'text-[#0ECB81]' : 'text-[#F6465D]'}`}>
          {isBullish ? '+' : ''}{profitLoss.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </div>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-gray-400">ROI</span>
          <span className={`text-sm font-semibold ${isBullish ? 'text-[#0ECB81]' : 'text-[#F6465D]'}`}>
            {isBullish ? '+' : ''}{profitLossPercent.toFixed(2)}%
          </span>
        </div>
      </div>
