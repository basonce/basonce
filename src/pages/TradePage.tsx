...
export default function TradePage() {
  ...
  const [currentPrice, setCurrentPrice] = useState(() => {
    ...
    if (sym === 'EQ') return EarnQuestPriceManager.getInstance().getPrice() || 0.055;
    return 0;
  });
  const [change24h, setChange24h] = useState(() => {
    const sym = getInitialSymbol();
    if (sym === 'EQ') return EarnQuestPriceManager.getInstance().getChange() || 0;
    return 0;
  });
  const [high24h, setHigh24h] = useState(() => {
    const sym = getInitialSymbol();
    if (sym === 'EQ') return EarnQuestPriceManager.getInstance().getHigh24h() || 0;
    return 0;
  });
  const [low24h, setLow24h] = useState(() => {
    const sym = getInitialSymbol();
    if (sym === 'EQ') return EarnQuestPriceManager.getInstance().getLow24h() || 0;
    return 0;
  });
  const [volume24h, setVolume24h] = useState(() => {
    const sym = getInitialSymbol();
    if (sym === 'EQ') return EarnQuestPriceManager.getInstance().getMarketCap() || 0;
    return 0;
  });
  const [volumeBase, setVolumeBase] = useState(() => {
    const sym = getInitialSymbol();
    if (sym === 'EQ') return (EarnQuestPriceManager.getInstance().getMarketCap() || 0) / 2;
    return 0;
  });
  ...
  const changeSymbol = (newSymbol: string) => {
    ...
    if (newSymbol === 'EQ') {
      ...
      const eqP = pm.getPrice();
      const eqC = pm.getChange();
      setCurrentPrice(eqP);
      setChange24h(eqC);
      setHigh24h(pm.getHigh24h());
      setLow24h(pm.getLow24h());
      setVolume24h(pm.getMarketCap());
      setVolumeBase(pm.getMarketCap() / 2);
      setPrice(eqP.toFixed(getPriceDecimals(eqP)));
    } else {
    ...
    for (const coin of allCoins) {
      if (coin.symbol === 'EQ') {
        prices[coin.symbol] = {
          ...
          change: priceManager.current.getChange()
        };
      } else {
  ...
  const fetchInitialData = async () => {
    ...
    if (targetSymbol === 'EQ') {
      ...
      const change = priceManager.current.getChange();
      setCurrentPrice(eqPrice);
      setChange24h(change);
      setHigh24h(priceManager.current.getHigh24h());
      setLow24h(priceManager.current.getLow24h());
      setVolume24h(priceManager.current.getMarketCap());
      setVolumeBase(priceManager.current.getMarketCap() / 2);
      setPrice(eqPrice.toFixed(getPriceDecimals(eqPrice)));
      generateOrderBook(eqPrice);
      generateInitialTrades(eqPrice);
    } else {
      try {
        ...
        if (depth && depth.bids.length > 0 && depth.asks.length > 0) {
          const bids: OrderBookItem[] = depth.bids.map(([price, amount]) => {
  ...
  const updatePrice = async () => {
    ...
    if (target === 'EQ') {
      const p = priceManager.current.getPrice();
      const c = priceManager.current.getChange();
      setCurrentPrice(p);
      setChange24h(c);
  ...
  const generateOrderBook = (basePrice?: number) => {
    ...
    const spreadStep = p < 1 ? p * 0.002 : p < 100 ? p * 0.001 : p < 10000 ? p * 0.0005 : p * 0.0002;

    for (let i = 0; i < 20; i++) {
      const jitter = 1 + (Math.random() - 0.5) * 0.3;
      const bidPrice = p - (i + 1) * spreadStep * jitter;
      const baseAmount = p > 1000 ? 0.01 + Math.random() * 0.5 : p > 10 ? 1 + Math.random() * 50 : 10 + Math.random() * 500;
      const depthFactor = 1 + i * 0.15;
      const bidAmount = baseAmount * depthFactor * (0.5 + Math.random());
      bids.push({ price: bidPrice, amount: bidAmount, total: bidPrice * bidAmount });

      const askPrice = p + (i + 1) * spreadStep * (1 + (Math.random() - 0.5) * 0.3);
      const askAmount = baseAmount * depthFactor * (0.5 + Math.random());
  ...
  const jitterOrderBook = (p: number) => {
    const step = p < 1 ? p * 0.002 : p < 100 ? p * 0.001 : p < 10000 ? p * 0.0005 : p * 0.0002;
    setBidOrders(prev => {
      if (prev.length === 0) { generateOrderBook(p); return prev; }
      return prev.map((order, i) => {
        const newAmount = Math.max(0.001, order.amount * (0.85 + Math.random() * 0.3));
        const newPrice = p - (i + 1) * step * (0.9 + Math.random() * 0.2);
        return { price: newPrice, amount: newAmount, total: newPrice * newAmount };
      });
    });
    setAskOrders(prev => {
      ...
      <div className="min-h-screen bg-[#181A20] text-white pb-20 max-w-[480px] mx-auto">
        ...
        <div className="grid grid-cols-[58%_42%] gap-0">
          ...
          <div className="bg-[#181A20] border-l border-[#2B3139]">
            ...
            <div className="px-1">
              {askOrders.slice(0, 8).reverse().map((order, index) => {
                const maxAmount = Math.max(...askOrders.slice(0, 8).map(o => o.amount));
                const fillPct = maxAmount > 0 ? (order.amount / maxAmount) * 100 : 0;
                return (
                  <div key={`ask-${index}`} className="ob-row-ask flex items-center justify-between py-[2.5px] px-1 transition-all duration-150" style={{ '--fill': `${fillPct}%` } as React.CSSProperties}>
                    <span className="text-[#F6465D] text-[11px] font-medium relative z-10 tabular-nums">{formatPrice(order.price)}</span>
                    <span className="text-[#EAECEF] text-[11px] relative z-10 tabular-nums">{formatAmount(order.amount)}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="px-1">
              {bidOrders.slice(0, 8).map((order, index) => {
                const maxAmount = Math.max(...bidOrders.slice(0, 8).map(o => o.amount));
                const fillPct = maxAmount > 0 ? (order.amount / maxAmount) * 100 : 0;
                return (
                  <div key={`bid-${index}`} className="ob-row-bid flex items-center justify-between py-[2.5px] px-1 transition-all duration-150" style={{ '--fill': `${fillPct}%` } as React.CSSProperties}>
                    <span className="text-[#0ECB81] text-[11px] font-medium relative z-10 tabular-nums">{formatPrice(order.price)}</span>
                    <span className="text-[#EAECEF] text-[11px] relative z-10 tabular-nums">{formatAmount(order.amount)}</span>
                  </div>
                );
              })}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-0.5">
                  <span className="text-[#848E9C] text-[10px]">
                    {currentPrice > 1000 ? '0.01' : currentPrice > 10 ? '0.001' : '0.0001'}
                  </span>
              ...
              <div className="px-4 py-2">
                {filteredCoins.map((coin) => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-10 h-10 rounded-full bg-[#2B3139] flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {coin.logo ? (
                          <img
                            src={getProxiedLogoUrl(coin.logo)}
                            alt={coin.symbol}
                            ...
                            onError={(e) => {
                              ...
                              if (parent) {
                                parent.innerHTML = `<span class="text-white font-bold text-sm">${coin.symbol.slice(0, 2)}</span>`;
                              }
                            }}
                          />
                        ) : (
      ...
      <div className="bg-[#181A20] px-4 py-3 sticky top-0 z-20 border-[#2B3139]">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            >
              {allCoins.find(c => c.symbol === selectedSymbol)?.logo && (
                <img
                  src={allCoins.find(c => c.symbol === selectedSymbol)?.logo}
                  alt={selectedSymbol}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="px-4 py-2">
              {filteredCoins.map((coin) => (
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-[#2B3139] flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {coin.logo ? (
                      <img
                        src={coin.logo}
                        alt={coin.symbol}
                        ...
                        onError={(e) => {
                          ...
                          const parent = target.parentElement;
                          if (parent) {
                            parent.innerHTML = `<span class="text-white font-bold text-xs">${coin.symbol.slice(0, 2)}</span>`;
                          }
                        }}
                      />
                    ) : (
