    ...
    try {
      ...
      const balancesWithPrices = await Promise.all(
        SUPPORTED_COINS.map(async (coin) => {
          ...
          if (coin.symbol === 'EQ' || coin.symbol === 'EQL') {
            try {
              const { EarnQuestPriceManager } = await import('../lib/earnquest-price');
              ...
              return {
                ...
                priceChange24h: priceManager.getChange()
              };
            } catch {
              return {
                symbol: coin.symbol,
          ...
          try {
            ...
            console.warn(`Failed to fetch price for ${coin.symbol}:`, error);
            return {
              symbol: coin.symbol,
              balance: userBalance.balance,
              locked_balance: userBalance.locked,
        ...
        <div className="mb-3">
          <div className="flex items-baseline gap-2 mb-1">
            ...
            <div className="relative">
              ...
              >
                <span className="text-xl font-black">{selectedCurrency}</span>
                ...
                <div key={coin.symbol} className="bg-[#181A20] rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      {coinInfo?.logo && (
                        <img src={getProxiedLogoUrl(coinInfo.logo)} alt={coin.symbol} className="w-10 h-10 rounded-full" />
                      )}
                      <div>
