...
export function getCoinLogoUrl(symbol: string, dbLogos?: Record<string, string>): string | null {
  if (symbol === 'EQ') return '/earnquest-logo-icon-2.png';
  if (dbLogos?.[symbol]) return dbLogos[symbol];
...
export function hasCoinLogo(symbol: string, dbLogos?: Record<string, string>): boolean {
  return getCoinLogoUrl(symbol, dbLogos) !== null;
}
