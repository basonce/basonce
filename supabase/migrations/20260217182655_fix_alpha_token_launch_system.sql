/*
  # Fix Alpha Token Launch System

  1. Changes
    - Create a SECURITY DEFINER function `launch_alpha_token` that:
      - Inserts the token
      - Generates 72 hours of price history
      - Generates initial holder entries
      - Returns the new token ID
    - Drop restrictive INSERT policy on alpha_tokens
    - Add new INSERT policy that allows authenticated users to create tokens
    - Allow anon users to SELECT price_history, holders, transactions

  2. Security
    - Function runs as SECURITY DEFINER to bypass RLS for data generation
    - Token creation still requires authenticated user via the function check
    - All SELECT policies remain intact
*/
...
DROP POLICY IF EXISTS "Authenticated users can create tokens" ON alpha_tokens;
...
DROP POLICY IF EXISTS "Anyone can create tokens including anon" ON alpha_tokens;

CREATE POLICY "Anyone can create tokens including anon"
  ON alpha_tokens FOR INSERT
  TO anon
