/*
  # Recreate Social Posts with Anonymous Profiles

  1. Purpose
    - Delete all existing social posts
    - Create 5000 new realistic social posts using anonymous_profiles
    - Real photos from 270 different people
    - Realistic trading scenarios and PnL
    
  2. Content
    - Mix of profitable and loss positions
    - Various coins (BTC, ETH, BNB, SOL, DOGE, XRP, etc.)
    - Leverage 5x-125x
...
-- Generate 5000 realistic social posts using anonymous profiles
DO $$
DECLARE
  i INTEGER;
  coin_symbol TEXT;
  ...
  loss_comments TEXT[] := ARRAY[
    ...
    'This one didn''t work out. Next trade.',
...
BEGIN
  FOR i IN 1..5000 LOOP
    ...
    -- Determine trade type (55% long, 45% short)
    trade_type := CASE WHEN random() < 0.55 THEN 'long' ELSE 'short' END;
    
    -- Random leverage (weighted towards lower leverage)
    leverage := CASE 
      WHEN random() < 0.4 THEN 5 + floor(random() * 15)::INTEGER  -- 40% chance: 5-20x
      WHEN random() < 0.75 THEN 20 + floor(random() * 30)::INTEGER -- 35% chance: 20-50x
      ELSE 50 + floor(random() * 75)::INTEGER  -- 25% chance: 50-125x
    END;
    
    -- Set realistic entry prices based on coin
    ...
    -- 65% profitable trades, 35% losses
    is_bullish := random() < 0.65;
    
    IF is_bullish THEN
      -- Profitable trade
      profit_loss_percent := 5 + (random() * 45); -- 5% to 50% profit
      ...
      -- PnL calculation (margin * profit_percent)
      profit_loss := (entry_price * leverage / 10) * (profit_loss_percent / 100);
    ELSE
      -- Loss trade
      profit_loss_percent := -(2 + (random() * 18)); -- -2% to -20% loss
    ...
    random_hours := floor(random() * 720)::INTEGER;
  ...
  RAISE NOTICE 'Successfully generated 5000 social posts with anonymous profiles';
END $$;
